...
export function RegisterTeacherForm({ schools }: { schools: School[] }) {
  ...
  async function onSubmit(values: z.infer<typeof teacherRegisterSchema>) {
    setError("")
    setSuccess("")
    setIsPending(true)
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto shadow-lg border-blue-100">
      <CardHeader className="space-y-1 text-center bg-blue-50/50 rounded-t-xl pb-6 border-b border-blue-100/50">
        <CardTitle className="text-2xl font-bold text-gray-800">Öğretmen Kaydı</CardTitle>
        <CardDescription className="text-gray-500">
          Sorumlu öğretmen olarak sisteme kayıt olun
        </CardDescription>
      </CardHeader>
      <CardContent className="pt-6">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="0555 555 55 55" disabled={isPending} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="md:col-span-2">
                    <FormLabel>Sorumlu Olduğunuz Okul</FormLabel>
            ...
            <Button 
              type="submit" 
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 shadow-md"
              disabled={isPending}
            >
              {isPending ? (
                <>
